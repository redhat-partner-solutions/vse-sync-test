...
To get the tools for data collection you need to clone the repository:
  ...
  git clone git@github.com:redhat-partner-solutions/vse-sync-collection-tools.git
  cd vse-sync-collection-tools
...
For convenience we will set up a few vars. The tool needs a `kubeconfig` with access to the `"openshift-ptp` namespace, the name of the PTP device, and a location to store output files.
  ...
  export COLLECTDEVICE=$(oc --kubeconfig ${KUBECONFIG} exec daemonset/linuxptp-daemon -c linuxptp-daemon-container -- ls /sys/class/gnss/gnss0/device/net/)
  export DATADIR=~/testdata
  mkdir $DATADIR

The collector tool includes a built-in tool that will allow you to perform some basic environment validations. These check the setup you have is one that is a valid test target, not that it is functioning correctly.

  go run main.go env verify --interface="${COLLECTDEVICE}" --kubeconfig="${KUBECONFIG}"

*Note that an Out-Of-Tree driver for the WPC card will result in an error of unknown driver version. This does not prevent the collection from being run.*

The collector tool will, by default, collect everything it can. For more specific details see the command help (`go run main.go --help`). The example command here will collect `1500` data samples at the default rate (once per second).

  go run main.go collect --interface="${COLLECTDEVICE}" --kubeconfig="${KUBECONFIG}" --output="${DATADIR}/collected.log" --use-analyser-format --duration=1500s
...
There is also a tool that will conveniently download logs of interest from the PTP operator and save them to a provided directory. You can do this yourself with `oc` if you prefer.
  ...
  go run main.go logs -k="${KUBECONFIG}" -o ${DATADIR} --since=1500s
...
These tools can be run as a single command that will collect for a duration and then retrieve all logs of interest from that duration (this example uses 1000 seconds):
  ...
  go run main.go collect --interface="${COLLECTDEVICE}" --kubeconfig="${KUBECONFIG}" --output="${DATADIR}/collected.log" --use-analyser-format --duration=1500s  && go run main.go logs -k="${KUBECONFIG}" -o ${DATADIR} --since=1500s
...
* TODO: write this section and rewrite the results section* 
  git clone git@github.com:redhat-partner-solutions/testdrive.git
