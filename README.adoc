...
For example to run analysis over an existing dataset stored in `~/tmp/data/` the follow command could be used:

[source,shell]
----
podman run -v ~/kubeconfig-cormorant:/usr/vse/kubeconfig -v ~/tmp/data:/usr/vse/data quay.io/redhat-partner-solutions/vse-sync-test:latest ./vse-sync-test/hack/e2e.sh -s
----
