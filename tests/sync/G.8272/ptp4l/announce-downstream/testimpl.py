...
sync/G.8272/ptp4l/announce-downstream

Checks that downstream Announce messages are sent around clock class changes.
"""
...
# Heuristics for downstream (transmit) Announce messages in ptp4l logs
RE_ANNOUNCE_TX = re.compile(
    r'(?i)\b(send|sending|tx).{0,32}\bannounce\b|'
    r'\bmsg(?:Type)?\s*(?:11|0x0b).{0,32}\b(tx|sent|sending)\b'
)
...
def refimpl(filename, encoding='utf-8'):
    """For each clock class change event, verify at least one downstream (transmit) Announce nearby."""
    class_change_times = []
    last_class = None
    ...
    window_seconds = 15
    tx_hits = {t: False for t in class_change_times}
    first_ts = None
    last_ts = None
    with open_input(filename, encoding=encoding) as fid:
        for line in fid:
            ...
            if ts:
                if first_ts is None:
                    first_ts = ts
                last_ts = ts
            if RE_ANNOUNCE_TX.search(line) and ts is not None:
                for change_ts in class_change_times:
                    if abs((ts - change_ts).total_seconds()) <= window_seconds:
                        tx_hits[change_ts] = True

    missing = [t.isoformat() + 'Z' for (t, hit) in tx_hits.items() if not hit]
    if missing:
        return {
            'result': False,
            'reason': 'missing downstream Announce near some clock changes',
